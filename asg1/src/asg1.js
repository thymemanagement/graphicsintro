...
"void main() {\n" +
" vec3 newPosition = (a_Size * a_Vertex) + a_Position;\n" + 
...
function connectVariablesToGLSL(gl, vshader, fshader) {
    ...
    return {a_Position: a_Position, a_Size: a_Size, u_FragColor: u_FragColor, vbuffer: vertexBuffer}
}
...
function renderAllShapes(gl, vars, points) {
    ...
    for (p in points) {
        let point = points[p]
        gl.vertexAttrib3f(vars.a_Position, point.x, point.y, 0.0)
        gl.vertexAttrib1f(vars.a_Size, point.size)
